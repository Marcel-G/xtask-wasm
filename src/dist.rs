use crate::{
    anyhow::{ensure, Context, Result, Error},
...
pub struct Dist {
    ...
    /// Should typescript .d.ts be output
    #[clap(long)]
    pub typescript: bool,

    /// Target for output: Web, Bundler, NodeJS, NoModules, Deno
    #[clap(long)]
...
impl Dist {
    ...
    pub fn run(self, package_name: &str) -> Result<DistResult> {
        ...
        let wasm_filename = format!("{app_name}_bg");
...
trait TargetExt {
    fn target(&mut self, target: Option<Target>) -> Result<&mut Self, Error>;
}

impl TargetExt for Bindgen {
    fn target(&mut self, target: Option<Target>) -> Result<&mut Self, Error> {
        match target {
            Some(Target::Web) => self.web(true),
            Some(Target::Bundler) => self.bundler(true),
            Some(Target::NodeJS) => self.nodejs(true),
            Some(Target::NoModules) => self.no_modules(true),
            Some(Target::Deno) => self.deno(true),
            None => self.web(true)
        }
    }
}
